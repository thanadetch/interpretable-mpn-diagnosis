"""
UNI2-h Feature Extraction — Reticulin-Stained Images (Silver Stain).

Simplified version for Reticulin images that skips color metric calculations
(originally designed for H&E). Extracts frozen UNI2-h [CLS] embeddings
from pre-patched image tiles, grouped by source image ID. Each original image
produces one .pt file containing a stacked tensor of shape [N_patches, 1536].

Input:  data/processed_grading/{Class}/{PatientID}/{ImgID}_r{Row}c{Col}.png
Output: data/features_uni2_reti/{Class}/{PatientID}/{ImgID}.pt

Usage:
    python -m src.tools.extract_uni2_reti \
        --data_dir data/processed_grading \
        --output_dir data/features_uni2_reti \
        --batch_size 64 \
        --device cuda

    # Dry-run (validates grouping without extracting):
    python -m src.tools.extract_uni2_reti --dry_run
"""
...
# Ensure src/ is on sys.path when running directly (e.g., python src/tools/extract_uni2_reti.py)
...
DEFAULT_OUTPUT_DIR = "data/features_uni2_reti"
FEATURE_DIM = 1536
CLASSES = ["ET", "PV", "PMF"]
...
def extract_features_for_image(
    ...
    Returns:
        Tuple of (features [N_patches, 1536], rc [N, 2] int32).
    """
...
def run_extraction(args: argparse.Namespace) -> None:
    ...
    # ------------------------------------------------------------------
    print("=" * 60)
    print("UNI2-h Feature Extraction Plan (Reticulin)")
    print("=" * 60)
    ...
    # ------------------------------------------------------------------
    print(f"\n{'=' * 60}")
    print("UNI2-h Feature Extraction Complete (Reticulin)")
    print(f"{'=' * 60}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract UNI2-h [CLS] features for Reticulin-stained images."
    )
    parser.add_argument(
        "--data_dir",
        type=str,
