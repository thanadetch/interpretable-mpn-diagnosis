"""
Virchow2 Feature Extraction — Reticulin-Stained Images (Silver Stain).

Simplified version for Reticulin images that skips color metric calculations
(originally designed for H&E). Extracts frozen Virchow2 [CLS] embeddings
from pre-patched image tiles, grouped by source image ID. Each original image
produces one .pt file containing a stacked tensor of shape [N_patches, 1280].

Input:  data/processed_grading/{Class}/{PatientID}/{ImgID}_r{Row}c{Col}.png
Output: data/features_virchow2_reti/{Class}/{PatientID}/{ImgID}.pt

Usage:
    python -m src.tools.extract_virchow2_reti \
        --data_dir data/processed_grading \
        --output_dir data/features_virchow2_reti \
        --batch_size 64 \
        --device cuda

    # Dry-run (validates grouping without extracting):
    python -m src.tools.extract_virchow2_reti --dry_run
"""
...
# Ensure src/ is on sys.path when running directly (e.g., python src/tools/extract_virchow2_reti.py)
...
DEFAULT_OUTPUT_DIR = "data/features_virchow2_reti"
FEATURE_DIM = 1280
CLASSES = ["ET", "PV", "PMF"]
...
def extract_features_for_image(
    ...
    Returns:
        Tuple of (features [N_patches, 1280], rc [N, 2] int32).
    """
    ...
    for batch_tensor, batch_rows, batch_cols in loader:
        ...
        with torch.autocast(device.type, torch.float16, enabled=use_amp):
            output = model(batch_tensor)  # [B, 261, 1280]
    ...
    concatenated_features = torch.cat(all_features, dim=0)  # [N, 1280]
...
def run_extraction(args: argparse.Namespace) -> None:
    ...
    # ------------------------------------------------------------------
    print("=" * 60)
    print("Virchow2 Feature Extraction Plan (Reticulin)")
    print("=" * 60)
    ...
    # ------------------------------------------------------------------
    print(f"\n{'=' * 60}")
    print("Virchow2 Feature Extraction Complete (Reticulin)")
    print(f"{'=' * 60}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Virchow2 [CLS] features for Reticulin-stained images."
    )
    parser.add_argument(
        "--data_dir",
        type=str,
